...
static struct acl *conf_parse_acls(json_t *jtab);
...
static struct auth *conf_auth_username_password(json_t *jarray);

int conf_str_allcaps(const char *s, const size_t sz) {
  size_t i;
  for (i = 0; i < sz; i++) {
    if (s[i] != toupper(s[i])) {
      return 0;
...
struct conf *conf_read(const char *filename) {
  ...
  for (kv = json_object_iter(j); kv; kv = json_object_iter_next(j, kv)) {
    ...
    if (strcmp(json_object_iter_key(kv), "redis_host") == 0 &&
        json_typeof(jtmp) == JSON_STRING) {
      free(conf->redis_host);
      conf->redis_host = conf_string_or_envvar(json_string_value(jtmp));
    } else if (strcmp(json_object_iter_key(kv), "redis_port") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      conf->redis_port = (int)json_integer_value(jtmp);
    } else if (strcmp(json_object_iter_key(kv), "redis_port") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->redis_port =
          atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "redis_auth") == 0) {
      if (json_typeof(jtmp) == JSON_STRING) {
        conf->redis_auth =
            conf_auth_legacy(conf_string_or_envvar(json_string_value(jtmp)));
      } else if (json_typeof(jtmp) == JSON_ARRAY) {
        conf->redis_auth = conf_auth_username_password(jtmp);
      } else if (json_typeof(jtmp) != JSON_NULL) {
        fprintf(
            stderr, ACL_ERROR_PREFIX
            "expected a string or an array of two strings" ACL_ERROR_SUFFIX);
      }
    } else if (strcmp(json_object_iter_key(kv), "http_host") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      free(conf->http_host);
      conf->http_host = conf_string_or_envvar(json_string_value(jtmp));
    } else if (strcmp(json_object_iter_key(kv), "http_port") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      ...
      conf->http_port =
          atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "http_max_request_size") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      conf->http_max_request_size = (size_t)json_integer_value(jtmp);
    } else if (strcmp(json_object_iter_key(kv), "http_max_request_size") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->http_max_request_size =
          (size_t)atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "threads") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      ...
      conf->http_threads =
          atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "acl") == 0 &&
               json_typeof(jtmp) == JSON_ARRAY) {
      conf->perms = conf_parse_acls(jtmp);
    } else if (strcmp(json_object_iter_key(kv), "user") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      struct passwd *u;
      if ((u = getpwnam(conf_string_or_envvar(json_string_value(jtmp))))) {
        conf->user = u->pw_uid;
      }
    } else if (strcmp(json_object_iter_key(kv), "group") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      struct group *g;
      if ((g = getgrnam(conf_string_or_envvar(json_string_value(jtmp))))) {
        conf->group = g->gr_gid;
      }
    } else if (strcmp(json_object_iter_key(kv), "logfile") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->logfile = conf_string_or_envvar(json_string_value(jtmp));
    } else if (strcmp(json_object_iter_key(kv), "log_fsync") == 0) {
      if (json_typeof(jtmp) == JSON_STRING &&
          strcmp(json_string_value(jtmp), "auto") == 0) {
        conf->log_fsync.mode = LOG_FSYNC_AUTO;
      } else if (json_typeof(jtmp) == JSON_STRING &&
                 strcmp(json_string_value(jtmp), "all") == 0) {
        conf->log_fsync.mode = LOG_FSYNC_ALL;
      } else if (json_typeof(jtmp) == JSON_INTEGER &&
                 json_integer_value(jtmp) > 0) {
        conf->log_fsync.mode = LOG_FSYNC_MILLIS;
        conf->log_fsync.period_millis = (int)json_integer_value(jtmp);
      } else if (json_typeof(jtmp) != JSON_NULL) {
        fprintf(stderr,
                "Unexpected value for \"log_fsync\", defaulting to \"auto\"\n");
      }
    } else if (strcmp(json_object_iter_key(kv), "verbosity") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      int tmp = json_integer_value(jtmp);
      if (tmp < 0 || tmp > (int)WEBDIS_TRACE) {
        fprintf(stderr,
                "Invalid log verbosity: %d. Acceptable range: [%d .. %d]\n",
                tmp, WEBDIS_ERROR, WEBDIS_TRACE);
      }
      conf->verbosity =
          (tmp < 0 ? WEBDIS_ERROR
                   : (tmp > WEBDIS_TRACE ? WEBDIS_TRACE : (log_level)tmp));
    } else if (strcmp(json_object_iter_key(kv), "daemonize") == 0 &&
               json_typeof(jtmp) == JSON_TRUE) {
      conf->daemonize = 1;
    } else if (strcmp(json_object_iter_key(kv), "daemonize") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->daemonize =
          is_true_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "pidfile") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->pidfile = conf_string_or_envvar(json_string_value(jtmp));
    } else if (strcmp(json_object_iter_key(kv), "websockets") == 0 &&
               json_typeof(jtmp) == JSON_TRUE) {
      conf->websockets = 1;
    } else if (strcmp(json_object_iter_key(kv), "websockets") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->websockets =
          is_true_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "database") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      ...
      conf->database =
          atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "pool_size") == 0 &&
               json_typeof(jtmp) == JSON_INTEGER) {
      conf->pool_size_per_thread = json_integer_value(jtmp);
    } else if (strcmp(json_object_iter_key(kv), "pool_size") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
      conf->pool_size_per_thread =
          atoi_free(conf_string_or_envvar(json_string_value(jtmp)));
    } else if (strcmp(json_object_iter_key(kv), "default_root") == 0 &&
               json_typeof(jtmp) == JSON_STRING) {
...
void acl_read_commands(json_t *jlist, struct acl_commands *ac) {
...
struct acl *conf_parse_acl(json_t *j) {
  ...
  if ((jcidr = json_object_get(j, "ip")) && json_typeof(jcidr) == JSON_STRING) {
    ...
    a->cidr.enabled = 1;
    a->cidr.mask =
        (mask_bits == 0 ? 0xffffffff : (0xffffffff << (32 - mask_bits)));
    a->cidr.subnet = ntohl(inet_addr(ip)) & a->cidr.mask;
    free(ip);
  }

  /* parse basic_auth */
  if ((jbasic = json_object_get(j, "tenant_id")) &&
      json_typeof(jbasic) == JSON_STRING) {
  ...
  if ((jlist = json_object_get(j, "enabled")) &&
      json_typeof(jlist) == JSON_ARRAY) {
    acl_read_commands(jlist, &a->enabled);
  }

  /* parse disabled commands */
  if ((jlist = json_object_get(j, "disabled")) &&
      json_typeof(jlist) == JSON_ARRAY) {
...
struct acl *conf_parse_acls(json_t *jtab) {
...
static struct auth *conf_auth_legacy(char *password) {
  ...
  if (!ret) {
    free(password);
    fprintf(stderr, ACL_ERROR_PREFIX
            "failed to allocate memory for credentials" ACL_ERROR_SUFFIX);
    return NULL;
  }
  ret->use_legacy_auth = 1;
  ret->password =
      password; /* already transformed to include decoded env vars */
  return ret;
}
...
static struct auth *conf_auth_username_password(json_t *jarray) {
  ...
  if (array_size != 2) {
    fprintf(stderr,
            ACL_ERROR_PREFIX "expected two values, found %zu" ACL_ERROR_SUFFIX,
            array_size);
    return NULL;
  }
  json_t *jusername = json_array_get(jarray, 0);
  json_t *jpassword = json_array_get(jarray, 1);
  if (json_typeof(jusername) != JSON_STRING ||
      json_typeof(jpassword) != JSON_STRING) {
    fprintf(stderr,
            ACL_ERROR_PREFIX "both values need to be strings" ACL_ERROR_SUFFIX);
    return NULL;
  }
